...
describe("parseVerdict", () => {
  ...
  it("rejects a missing strengths field", () => {
    const bad = JSON.stringify({ weaknesses: ["a", "b"], recommendation: "x" });
    expect(() => parseVerdict(bad, "m")).toThrow(/strengths/);
  });

  it("rejects strengths with the wrong number of items", () => {
    const bad = JSON.stringify({ strengths: ["only one"], weaknesses: ["a", "b"], recommendation: "x" });
    expect(() => parseVerdict(bad, "m")).toThrow(/strengths/);
  });

  it("rejects an empty-string item", () => {
    const bad = JSON.stringify({ strengths: ["a", "   "], weaknesses: ["a", "b"], recommendation: "x" });
    expect(() => parseVerdict(bad, "m")).toThrow(/strengths/);
  });

  it("rejects a missing recommendation", () => {
    const bad = JSON.stringify({ strengths: ["a", "b"], weaknesses: ["a", "b"] });
