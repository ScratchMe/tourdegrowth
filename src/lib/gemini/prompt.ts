...
const OUTPUT_INSTRUCTION: Record<Locale, string> = {
  en: 'Respond with ONLY a valid JSON object, no markdown fences, no text before or after, matching exactly this schema (keys stay in English, only the VALUES are in the requested language):\n{"headline": "<one short, punchy sentence summarizing the overall result — must work standalone on a phone screen, under 60 characters>", "strengths": ["<pillar-grounded strength 1>", "<pillar-grounded strength 2>"], "weaknesses": ["<pillar-grounded improvement area 1>", "<pillar-grounded improvement area 2>"], "recommendation": "<one specific, prioritized next action>"}',
  fr: 'Réponds UNIQUEMENT avec un objet JSON valide, sans balises markdown, sans texte avant ou après, respectant exactement ce schéma (les clés restent en anglais, seules les VALEURS sont dans la langue demandée) :\n{"headline": "<une phrase courte et percutante résumant le résultat global — doit fonctionner seule sur un écran de téléphone, moins de 60 caractères>", "strengths": ["<force ancrée dans un pilier 1>", "<force ancrée dans un pilier 2>"], "weaknesses": ["<axe d\'amélioration ancré dans un pilier 1>", "<axe d\'amélioration ancré dans un pilier 2>"], "recommendation": "<une action prioritaire précise>"}',
};
