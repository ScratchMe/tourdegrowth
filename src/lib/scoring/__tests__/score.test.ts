...
/** Builds a full 15-answer Answers record from indices in canonical question order. */
...
describe("computeScore", () => {
  it("scores a perfect run as 100/100, 20/20 on every pillar", () => {
    const result = computeScore(answersFrom(fill(3)));
    expect(result.total).toBe(100);
    for (const p of result.pillars) expect(p.score).toBe(20);
  });

  it("scores a zero run as 0/100, 0/20 on every pillar", () => {
  ...
  it("scales raw points (/60) to a pillar score (/20)", () => {
    // acquisition: 13 + 20 + 20 = 53 raw -> 53/3 = 17.67 -> rounds to 18
    const answers = answersFrom(fill(0));
    answers["acquisition-1"] = 2;
    answers["acquisition-2"] = 3;
    answers["acquisition-3"] = 3;
    const result = computeScore(answers);
    const acquisition = result.pillars.find((p) => p.pillar === "acquisition");
    expect(acquisition).toEqual({ pillar: "acquisition", rawPoints: 53, score: 18 });
  });

  it("rounds each pillar BEFORE summing — the total is not a recomputed average of raw points", () => {
    // Every pillar answered 7+7+20=34 raw points -> each pillar rounds to 11 (34/3 = 11.33).
    // Correct total per SPEC.md §6: sum the 5 already-rounded 11s = 55.
    // The wrong approach (sum all raw points first, then scale/round once)
    // would give round(170/300*100) = 57 instead — a different number,
    // which is exactly the bug SPEC.md §6 rules out.
    const answers = answersFrom(
      PILLARS.flatMap(() => [1, 1, 3] as AnswerIndex[]),
    );
    const result = computeScore(answers);

    for (const p of result.pillars) {
      expect(p.rawPoints).toBe(34);
      expect(p.score).toBe(11);
    }
  ...
  it("every pillar score is always an integer (never e.g. 07.5/20)", () => {
    // Exhaustively check every reachable raw-points sum for this pillar's 3 questions.
    for (const a of ANSWER_POINTS) {
      for (const b of ANSWER_POINTS) {
        for (const c of ANSWER_POINTS) {
          const raw = a + b + c;
  ...
  it("throws a descriptive error when an answer is missing", () => {
    ...
    delete answers["acquisition-2"];
    expect(() => computeScore(answers)).toThrow(/acquisition-2/);
  });

  it("throws a descriptive error when an answer index is invalid", () => {
    const answers = answersFrom(fill(0)) as Record<string, number>;
    answers["revenue-3"] = 4;
    expect(() => computeScore(answers as Answers)).toThrow(/revenue-3/);
  });
  ...
  it("picks the weakest pillar using the canonical AARRR tie-break", () => {
    ...
    const weak = computeScore(
      answersFrom([
        ...([0, 0, 0] as AnswerIndex[]), // acquisition: 0 raw -> 0
        ...([3, 3, 3] as AnswerIndex[]), // activation: 60 raw -> 20
        ...([3, 3, 3] as AnswerIndex[]), // retention
        ...([3, 3, 3] as AnswerIndex[]), // referral
        ...([3, 3, 3] as AnswerIndex[]), // revenue
      ]),
    );
    ...
    const tie = computeScore(
      answersFrom([
        ...([3, 3, 3] as AnswerIndex[]), // acquisition: 20
        ...([0, 0, 0] as AnswerIndex[]), // activation: 0
        ...([0, 0, 0] as AnswerIndex[]), // retention: 0 (tied with activation)
        ...([3, 3, 3] as AnswerIndex[]), // referral
        ...([3, 3, 3] as AnswerIndex[]), // revenue
      ]),
    );
