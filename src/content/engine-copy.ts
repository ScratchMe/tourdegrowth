...
import type { SlideTitleKey, ToolId } from "@/lib/engine/types";
 ...
 *
 * **Every key exists now with provisional wording taken from the spec**, so
 * the UI (P4-P6) builds against real keys while the content PR (P3) rewrites
 * the values. Nothing here is approved: the whole file goes to bon à tirer
 * nº6, rebuilt from `grep -rn "TODO: à relire" src/`.
 *
 ...
 *   `lib/engine/format.ts` — a template never formats a number itself;
 * - `**…**` marks the red accent of a slide title (one per title, at most);
 * - grammatical number is a separate key: `xOne` is the singular form, `x`
 *   the general one (`coverage.found` / `coverage.foundOne`);
 * - glyphs stay inside the three fonts' coverage (§10.4): no arrows other
 *   than "→" in running copy the fonts carry, no "≈", no "✓", "1re" rather
 *   than a superscript.
 */
...
export const ENGINE_COPY = {
  ...
  page: {
    ...
    promise: {
      fr: "Quinze chiffres, trois par étape : trouve-les, vois où ton moteur perd du monde, et repars avec des slides prêtes pour ton CODIR.",
      en: "Fifteen numbers, three per stage: find them, see where your engine loses people, and leave with slides ready for your leadership meeting.",
    },
    privacyTitle: { fr: "Rien de ce que tu saisis ne sort d'ici", en: "Nothing you enter leaves this page" },
    privacyBody: {
      fr: "Aucun chiffre ni aucun texte que tu saisis ne quitte ton navigateur. Pas de compte, pas de serveur : ils restent sur cet appareil, et tu peux le vérifier dans l'onglet Réseau. La page compte ses visites, sans cookie — jamais ce que tu y écris.",
      en: "No number and no text you enter leaves your browser. No account, no server: they stay on this device, and you can check it in the Network tab. The page counts its visits, without cookies — never what you type.",
    },
    ...
    tourFirst: { fr: "Faire le Tour d'abord (3 min)", en: "Take the Tour first (3 min)" },
    noscript: {
      fr: "Le moteur a besoin de JavaScript pour enregistrer tes chiffres. La liste des quinze chiffres ci-dessous se lit sans.",
      en: "The engine needs JavaScript to save your numbers. The list of fifteen numbers below reads without it.",
    },
    catalogueTitle: { fr: "Les quinze chiffres", en: "The fifteen numbers" },
    catalogueIntro: {
      fr: "Trois par étape, comme les trois questions du Tour. Pour chacun : sa formule, où le trouver, et ce qu'il faut savoir avant de le citer.",
      en: "Three per stage, like the Tour's three questions. For each: its formula, where to find it, and what to know before quoting it.",
    },
    catalogueComputedTitle: { fr: "Et trois chiffres calculés", en: "And three computed numbers" },
    catalogueVerified: { fr: "Chemins de menu vérifiés en {month}.", en: "Menu paths checked in {month}." },
  ...
  setup: {
    ...
    models: {
      selfserve: { fr: "SaaS / produit web en libre-service (essai ou freemium)", en: "SaaS / web product, self-serve (trial or freemium)" },
      salesLed: { fr: "B2B avec équipe commerciale", en: "B2B with a sales team" },
    ...
    referenceMonthHint: {
      fr: "Visiteurs, inscriptions, dépense, churn et ARPA de ce mois-là. Par défaut : le dernier mois clos.",
      en: "Visitors, sign-ups, spend, churn and ARPA for that month. Default: the last closed month.",
    },
    cohortMonth: { fr: "Cohorte suivie", en: "Cohort you follow" },
    cohortHint: {
      fr: "On suit les inscrits de {cohort} : ceux de {next} n'ont pas encore eu {n} jours.",
      en: "We follow {cohort}'s sign-ups: {next}'s haven't had {n} days yet.",
    },
    ...
    companyHint: { fr: "Reste sur cet appareil.", en: "Stays on this device." },
    tourFound: {
      fr: "Tu as fait le Tour le {date} ({score}/100). On comparera ce que tu y as déclaré à ce que tu retrouves ici.",
      en: "You took the Tour on {date} ({score}/100). We'll compare what you declared there with what you find here.",
    },
    tourLink: { fr: "Comparer avec mon Tour", en: "Compare with my Tour" },
    start: { fr: "Commencer →", en: "Start →" },
  },

  board: {
    eyebrow: {
      fr: "Ton moteur de croissance · {model} · cohorte de {cohort} · flux de {month}",
    ...
    pillsSummary: { fr: "{found} trouvés, {missing} introuvables, {inProgress} en cours", en: "{found} found, {missing} missing, {inProgress} in progress" },
  ...
  actions: {
    deck: { fr: "Préparer mes slides →", en: "Prepare my slides →" },
  ...
  cause: {
    ...
    noAccess: { fr: "Ça existe, mais je n'y ai pas accès", en: "It exists, but I have no access" },
  ...
  sheet: {
    ...
    cohortToUse: {
      fr: "Prends la cohorte de {cohort} : celle de {next} n'a pas encore eu {n} jours.",
      en: "Use the {cohort} cohort: {next}'s hasn't had {n} days yet.",
    },
    immatureCohort: { fr: "Cohorte pas encore complète : le chiffre sera marqué approximatif.", en: "Cohort not complete yet: the number will be marked approximate." },
    ...
    rateOnlyHint: {
      fr: "Sans les deux comptes, le chiffre sera marqué approximatif.",
      en: "Without both counts, the number will be marked approximate.",
    },
    ...
    definitionNoteHint: {
      fr: "Par exemple « actif = au moins un projet modifié ». Elle apparaît dans l'annexe du deck.",
      en: "For example \"active = at least one project edited\". It appears in the deck's appendix.",
    },
    ...
    wideRange: {
      fr: "Une fourchette aussi large ne dit presque rien — c'est déjà une information.",
      en: "A range this wide says almost nothing — that is already information.",
    },
    ...
    referenceContext: { fr: "{range} · contexte seulement, {caveat}", en: "{range} · context only, {caveat}" },
    ...
    targetHint: {
      fr: "Une cible d'équipe sert de repère pour désigner un frein.",
      en: "A team target acts as the reference for naming a bottleneck.",
    },
    dependsOnEvent: { fr: "Il faut d'abord nommer l'événement d'activation.", en: "Name the activation event first." },
    declaredAtTour: {
  ...
  request: {
    ...
    copyGroup: { fr: "Copier une demande pour les {n} chiffres", en: "Copy one request for the {n} numbers" },
  ...
  collect: {
    ...
    hint: {
      fr: "Lance les demandes aujourd'hui, remplis le reste en attendant.",
      en: "Send the requests today, fill in the rest while you wait.",
    },
  ...
  diagnosis: {
    ...
    notEnoughBelow: {
      fr: "{stage} est sous son repère ; sans cible sur les autres étapes, impossible de dire si c'est la plus grosse fuite.",
      en: "{stage} is below its reference; without targets on the other stages, we can't say whether it's the biggest leak.",
    },
    levelBody: {
      fr: "Aucune étape n'est sous sa cible : le levier est le volume ou le prix.",
      en: "No stage is below its target: the lever is volume or price.",
    },
    blindOne: {
      fr: "{stages} n'est pas mesurée : le vrai frein peut s'y cacher.",
      en: "{stages} isn't measured: the real bottleneck may be hiding there.",
    },
    blind: {
      fr: "{stages} ne sont pas mesurées : le vrai frein peut s'y cacher.",
      en: "{stages} aren't measured: the real bottleneck may be hiding there.",
    },
    unpriced: {
      fr: "Aussi sous ta cible, non chiffré en € : {stages}",
      en: "Also below your target, not priced in €: {stages}",
    },
    noArpa: {
      fr: "Le churn n'est pas comparable aux autres étapes sans ARPA.",
      en: "Churn can't be compared with the other stages without ARPA.",
    },
    topOfFunnel: {
      fr: "La plus grosse perte en nombre est toujours en haut du tunnel ; ce n'est pas ce qui désigne un frein.",
      en: "The biggest loss in numbers is always at the top of the funnel; that's not what names a bottleneck.",
    },
    stampReference: { fr: "Sous le repère", en: "Below reference" },
    stampTarget: { fr: "Sous ta cible", en: "Below target" },
  ...
  whatIf: {
    ...
    todayFlow: { fr: "{rate} → {n} nouveaux payants par mois", en: "{rate} → {n} new paying customers a month" },
    ifFlow: { fr: "{stage} atteint {target}", en: "{stage} reaches {target}" },
    thenFlow: { fr: "{n} × {target}/{rate} = {m} (+{delta})", en: "{n} × {target}/{rate} = {m} (+{delta})" },
    timesFlow: { fr: "{arpa} → {amount} de MRR ajouté chaque mois", en: "{arpa} → {amount} of MRR added every month" },
    todayChurn: { fr: "{churn} de churn sur {base} clients payants", en: "{churn} churn on {base} paying customers" },
    thenChurn: {
      fr: "{base} × ({churn} − {target}) = {n} clients préservés par mois",
      en: "{base} × ({churn} − {target}) = {n} customers kept a month",
    },
    timesChurn: { fr: "{arpa} → {amount} de MRR préservé chaque mois", en: "{arpa} → {amount} of MRR kept every month" },
    annual: {
    ...
    multiplication: {
      fr: "Dans un funnel, les taux se multiplient : +20 % sur n'importe quelle étape donne +20 % de clients. Ce qui distingue les étapes, c'est l'écart à leur cible.",
      en: "In a funnel, rates multiply: +20% at any stage gives +20% customers. What sets stages apart is the gap to their target.",
    },
    ...
    targetTeam: { fr: "{value} (ta cible)", en: "{value} (your target)" },
  ...
  peloton: {
    ...
    aria: {
      fr: "{n} sur 100 inscrits {population} — {status}, {source}, cohorte de {cohort}",
    ...
    /** A column's stage as the subject of "… isn't measured". */
    unmeasured: {
  ...
  mirror: {
    ...
    noTour: {
      fr: "Fais le Tour pour comparer ce que ton équipe déclare à ce que tu trouves.",
      en: "Take the Tour to compare what your team declares with what you find.",
    },
    gone: {
  ...
  deck: {
    ...
    containsData: {
      fr: "Ces fichiers contiennent les chiffres que tu as saisis.",
      en: "These files contain the numbers you entered.",
    },
    ...
    pngFailed: {
      fr: "L'image n'a pas pu être créée dans ce navigateur. Le PDF fonctionne.",
      en: "The image couldn't be created in this browser. The PDF works.",
    },
    otherLanguageHint: {
      fr: "Pour un deck en anglais, passe la page en EN : tes chiffres te suivent.",
      en: "For a deck in French, switch the page to FR: your numbers follow you.",
    },
  },
  ask: {
    ...
    costTeam: { fr: "{weeks} semaines d'une équipe de {people}", en: "{weeks} weeks of a team of {people}" },
  ...
  slide: {
    ...
    dataPill: {
      fr: "Données : {m} mesurées · {a} approximatives · {x} introuvables",
      en: "Data: {m} measured · {a} approximate · {x} missing",
    },
    footer: {
      fr: "Cohorte d'inscrits de {cohort} · flux de {month} · sources : {tools}",
      en: "{cohort} sign-up cohort · {month} flows · sources: {tools}",
    },
  ...
  slideTitles: {
    ...
    leakNotEnoughBelow: {
      fr: "**{stage} est sous son repère.** Sans cible sur les autres étapes, impossible de dire si c'est la plus grosse fuite.",
      en: "**{stage} sits below its reference.** Without targets on the other stages, we can't say whether it's the biggest leak.",
    },
    leakLevel: {
      fr: "Aucune étape n'est sous sa cible : **le levier est le volume ou le prix**.",
      en: "No stage sits below its target: **the lever is volume or price**.",
    },
    visibility: {
    ...
    unitEconomicsUnknown: {
      ...
      en: "**We can't yet say what a customer is worth**: {input} is missing.",
    },
    mirror: {
    ...
    ask: {
      fr: "Nous demandons **{what}** pour amener {metric} de {current} à {target} d'ici {horizon}.",
      en: "We're asking for **{what}** to take {metric} from {current} to {target} by {horizon}.",
    },
    askMeasureFirst: {
      fr: "Nous demandons **{cost}** pour mesurer {metric} avant de décider où investir.",
      en: "We're asking for **{cost}** to measure {metric} before deciding where to invest.",
    },
  ...
  notes: {
    ...
    source: {
      fr: "D'où vient ce chiffre ? — {tool}, {period}, cohorte de {cohort}.",
      en: "Where does this number come from? — {tool}, {period}, {cohort} cohort.",
    },
    seasonal: {
      fr: "Et si c'est saisonnier ? — Un seul mois est mesuré ; la comparaison mois à mois viendra.",
      en: "What if it's seasonal? — Only one month is measured; month-on-month comparison will come.",
    },
  ...
  findings: {
    ...
    noDefinition: {
      fr: "Il n'existe pas de définition partagée de {metric} — tout chiffre qu'on en donnerait serait l'opinion de quelqu'un.",
      en: "There's no shared definition of {metric} — any number given for it would be someone's opinion.",
    },
    blindSpot: {
      fr: "L'équipe déclare suivre {metric} ; personne n'a pu le sortir.",
      en: "The team says it tracks {metric}; nobody could pull it.",
    },
    ...
    unitEcon: {
      fr: "Impossible de dire en combien de mois un client rembourse son coût : il manque {input}.",
      en: "We can't say how many months a customer takes to pay back their cost: {input} is missing.",
    },
    reconcile: {
    ...
    smallCohort: {
      fr: "Moins de 100 inscrits dans la cohorte : chaque inscrit pèse plus d'un point.",
      en: "Fewer than 100 sign-ups in the cohort: each one weighs more than a point.",
    },
    hiddenKnowledge: {
      fr: "Tu en sais plus que ton Tour ne le dit : {metric} est suivi.",
      en: "You know more than your Tour says: {metric} is tracked.",
    },
  },
  sanity: {
    numGtDen: { fr: "Plus de {num} que de {den} : un des deux n'est pas le bon.", en: "More {num} than {den}: one of the two isn't the right one." },
    retainedGtActivated: {
      fr: "Plus d'actifs à J30 que d'activés : ta définition d'activation est peut-être trop stricte.",
      en: "More active users at day 30 than activated ones: your activation definition may be too strict.",
    },
    paidGtRetained: {
      fr: "Plus de payants que d'actifs à J30 : paiement annuel d'avance ?",
      en: "More paying customers than active users at day 30: annual prepayment?",
    },
    churnHigh: { fr: "C'est bien un churn mensuel ?", en: "Is that really a monthly churn?" },
  ...
  storage: {
    backupWarning: {
      fr: "Ton moteur n'existe que dans ce navigateur. Safari efface les données d'un site non visité depuis 7 jours.",
      en: "Your engine only exists in this browser. Safari erases data from a site not visited for 7 days.",
    },
  ...
  io: {
    ...
    importPreview: { fr: "{company} · {month} · {n} chiffres sur {N}", en: "{company} · {month} · {n} of {N} numbers" },
    replace: { fr: "Remplacer celui de cet appareil", en: "Replace the one on this device" },
    cancel: { fr: "Annuler", en: "Cancel" },
    warnings: { fr: "Fichier ouvert avec {n} avertissements :", en: "File opened with {n} warnings:" },
    unknownVersion: {
  ...
  resume: {
    ...
    remind: { fr: "Relancer {role}", en: "Follow up with {role}" },
  },
  erase: {
    title: { fr: "Tout effacer", en: "Erase everything" },
    body: {
      fr: "Tes chiffres seront supprimés de cet appareil. Sauvegarde-les d'abord si tu veux les garder.",
      en: "Your numbers will be deleted from this device. Save them first if you want to keep them.",
    },
  ...
  faq: [
    {
      ...
      a: {
        fr: "Non. Ils sont enregistrés dans le stockage local de ton navigateur, sur cet appareil. Aucune requête ne les transporte ; les seules façons de les faire sortir sont un fichier que tu télécharges ou ce que tu copies toi-même.",
        en: "No. They're stored in your browser's local storage, on this device. No request carries them; the only ways out are a file you download or what you copy yourself.",
      },
    },
    {
      q: { fr: "Pourquoi des comptes plutôt que des pourcentages ?", en: "Why counts rather than percentages?" },
      a: {
        fr: "Parce qu'un pourcentage sans sa base ne se vérifie pas. « 144 sur 800 » se recompte ; « 18 % » ne dit pas de quoi.",
        en: "Because a percentage without its base can't be checked. \"144 out of 800\" can be recounted; \"18%\" doesn't say of what.",
      },
    },
    {
      q: { fr: "D'où viennent les repères ?", en: "Where do the references come from?" },
      a: {
        fr: "Seulement des ordres de grandeur déjà publiés et relus dans le glossaire du site, avec leur réserve. La plupart des étapes n'en ont pas : ta propre cible est alors la référence.",
        en: "Only orders of magnitude already published and reviewed in the site's glossary, with their caveat. Most stages have none: your own target is then the reference.",
      },
    },
    {
      q: { fr: "Que faire d'un chiffre introuvable ?", en: "What do I do with a number I can't find?" },
      a: {
        fr: "Le dire. Un chiffre introuvable est un constat : l'outil te demande pourquoi, ce que coûterait de le réparer, et le met sur une slide.",
        en: "Say so. A number you can't find is a finding: the tool asks why, what fixing it would cost, and puts it on a slide.",
      },
    },
    {
      q: { fr: "En quoi est-ce différent du Tour ?", en: "How is this different from the Tour?" },
      a: {
        fr: "Le Tour mesure en trois minutes si ton équipe suit ses chiffres. Le moteur te fait aller les chercher, et confronte les deux si tu as fait le Tour.",
        en: "The Tour measures in three minutes whether your team tracks its numbers. The engine has you go and get them, and compares the two if you've taken the Tour.",
      },
    },
  ],
};
