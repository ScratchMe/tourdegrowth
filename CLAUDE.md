...
- **Limite de l'environnement de build à documenter** : l'appel `POST .../generateContent` (la génération elle-même) reste bloqué silencieusement depuis ce sandbox de session (connexion établie, requête envoyée, aucune réponse jusqu'au timeout — testé jusqu'à 60s, HTTP/1.1 forcé, `Expect:` désactivé) alors que les autres routes de la même API passent très bien. Le proxy réseau de la session ne journalise aucun rejet de politique (`recentRelayFailures` vide) — ce n'est donc probablement pas une politique organisationnelle documentée, plutôt une restriction propre à cet environnement de développement. Le pipeline (76 tests unitaires, mocks pour Gemini/Firestore) est solide, mais **le tout premier appel `generateContent` réel reste à vérifier après déploiement sur Vercel** (étape 12) — pas de proxy restrictif entre un serveur Vercel et l'API Gemini.
