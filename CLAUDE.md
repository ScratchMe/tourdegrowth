...
**Au 2026-09-25, tout le travail depuis la reprise du 24 est en production**, en une seule PR groupée ([#164](https://github.com/ScratchMe/tourdegrowth/pull/164) ; demande d'Antoine : « aller en prod avec tout ça, mais avec un feature flag que moi seul pourrait activer »). Elle porte : le design system v3 (trois couches de jetons, monde nuit, data-viz sans bibliothèque, famille des pages de prose), la revue de copie v1 et sa typographie française corrigée à la source, l'audit SEO v1 (dont `/aarrr-vs-heart`), les campagnes de lancement v2 (`marketing/campaigns/`), le jeu « Le côté obscur » niveau 1 et le moteur de croissance, **tous deux fermés derrière leur drapeau** (`GAME_ENABLED`, `ENGINE_ENABLED`) et ouvrables **par Antoine seul** sur son navigateur depuis `/admin/preview` (cookie signé avec le mot de passe admin — voir l'entrée du journal). La copie revue part avec ses marqueurs `TODO: à relire` : le [bon à tirer nº6](https://claude.ai/artifact/QqdzB6sT2zKcqJu8xD2VLH) (la copie existante revue, 37 cartes) reste à trancher, et les corrections qu'il demandera suivront en production. Avant d'ouvrir le jeu ou le moteur **pour tout le monde** : les bons à tirer [nº7](https://claude.ai/artifact/65Y8Ft3PCsBR1HDq4bsQSu) (le jeu) et [nº8](https://claude.ai/artifact/DUjhtzC6kBBN37idxDr1hJ) (le moteur), puis la variable dans Vercel et un redéploiement.
...
| L'alerte CodeQL | **Fermée le 2026-09-24** : le `console.error` de `create-submission.ts` (et le `console.warn` de la route Deep dive) passent leurs valeurs en `%s` | Rien. |
...
| Image OG : tokens | **Plus recopiés à la main depuis le 2026-09-24** : `src/lib/og/tokens.ts` lit les primitives de `src/styles/tokens/tokens.ts`, et `token-sources.test.ts` échoue sur toute divergence avec le CSS | Rien. |
...
| Vercel Functions Storage | **Ouvert, mais le gros poste est traité.** Somme glissante sur 30 jours, insensible à la suppression des déploiements (vérifié par Antoine auprès de Vercel le 2026-09-15). **Vercel compte par route** : la fonction des pages de contenu était à 4,36 Mo comptée 92 fois, soit 94 % des 428,9 Mo d'un déploiement — dont ~2,0 Mo de copies identiques de notre bibliothèque de contenu. **La déduplication est faite** (trois fan-in de notre propre code, pas une limite de Turbopack) : 7,9 → 5,74 Mo local, 47,3 → 43,55 Mo sur disque, soit ~−27 % par route en extrapolant le rapport de l'export | Lire l'export du prochain déploiement pour le chiffre réel. **Les deux correctifs secondaires sont tranchés** (2026-09-15) : `maxDuration` **non** unifié (0,5 % du total facturé contre une dégradation du chemin d'échec de `/api/submissions`) ; `ignoreCommand` étendu aux merges « doc seule » : **fait le 2026-09-24**. **Correction du 2026-09-23** : le compteur suit le poids disque (154 × 47,3 Mo = 7,28 Go ≈ 7,29 affichés), pas le poids par route de l'export — ajouter des routes ne coûte presque rien, ajouter du code serveur si. Détail : `VERCEL.md` §1.6 et §2.2. Et tenir la cadence de merges (convention 13) |
...
| Bons à tirer nº6, nº7, nº8 | **Publiés le 2026-09-25**, décisions dans la collection `cards/` de chacun : [nº6](https://claude.ai/artifact/QqdzB6sT2zKcqJu8xD2VLH) (la copie existante revue, 37 cartes), [nº7](https://claude.ai/artifact/65Y8Ft3PCsBR1HDq4bsQSu) (le jeu : 35 cartes — le français déjà validé du prototype est grisé, seuls le français neuf ou corrigé et tout l'anglais sont à relire), [nº8](https://claude.ai/artifact/DUjhtzC6kBBN37idxDr1hJ) (le moteur : 67 cartes, la première est la question des deux repères qui désignent). Tous trois construits en lisant les vrais modules, jamais en recopiant | Les réponses d'Antoine, puis lever les marqueurs et réécrire ce qu'il marque « à changer ». |
| Design system → Claude Design | Le bundle est prêt (70 aperçus, 215 cellules, rendu vérifié) ; le pousser passe par `/design-sync`, que seul Antoine peut lancer depuis sa machine | Quand Antoine lance `/design-sync`. |
