...
**Au 2026-09-25, tout le travail depuis la reprise du 24 est sur la branche `claude/repo-technical-functional-audit-e9xr8t`, pas encore sur `main`** — donc pas en production, qui reste à l'état du 2026-09-23. Cette branche porte : le design system v3 (trois couches de jetons, monde nuit, data-viz sans bibliothèque, famille des pages de prose), la revue de copie v1 et sa typographie française corrigée à la source, l'audit SEO v1 (dont `/aarrr-vs-heart`), les campagnes de lancement v2 (`marketing/campaigns/`), le jeu « Le côté obscur » niveau 1 et le moteur de croissance, **tous deux fermés derrière leur drapeau** (`GAME_ENABLED`, `ENGINE_ENABLED` ; aperçu par `?game=preview` / `?engine=preview`). Ce qui attend Antoine avant un merge : le [bon à tirer nº6](https://claude.ai/artifact/QqdzB6sT2zKcqJu8xD2VLH) (la copie existante revue, 37 cartes) et les décisions listées dans le tableau ci-dessous ; avant d'ouvrir le jeu ou le moteur, les bons à tirer [nº7](https://claude.ai/artifact/65Y8Ft3PCsBR1HDq4bsQSu) (le jeu) et [nº8](https://claude.ai/artifact/DUjhtzC6kBBN37idxDr1hJ) (le moteur). Un merge sur `main` de cette taille est **un** déploiement (convention 13) : c'est la PR groupée, quand Antoine la demande.
