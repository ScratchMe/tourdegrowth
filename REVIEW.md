...
**Réglé depuis** : le dépôt est passé public le 2026-09-05 (ce qui lève la restriction du plan Free) et Antoine a activé la règle — `main` répond `protected: true` à l'API GitHub, vérifié le 2026-09-08. Le check `Types, tests, build` est donc obligatoire ; `verify-live.yml` ne doit **jamais** l'être (il ne se déclenche pas sur `pull_request`, l'exiger bloquerait tous les merges).
