...
components/             — 27 components
  ...
  result/               — ScoreDisplay, PillarChip, InsightCard, Bottleneck,
                          PriorityMove, ShareCard, ToneToggle, Disclaimer, ScoreBreakdown
  share/                — ShareImage (Satori og:image layout, hex constants)
guidelines/             — visual reference boards, voice.md, compositions-ext-03.md
```
...
## Extension 03 — the result says what to do next

New: `Bottleneck` (the stage holding you back, stamped under the numeral inside the score card; three sharpness states), `ShareCard` (the og:image on its own page, with Share and Save), `ShareImage` (the og:image layout for Satori — pillar rows out, next move in). Changed: `PriorityMove` carries the free action for everyone and an owner-only `upgrade` slot; `ToneToggle` is rebased on `Segmented` and gains `size="compact"` for the landing preview card. Tokens: `--ratio-share`, `--pad-share-frame`. No new hex.

Page arrangements are in `guidelines/compositions-ext-03.md`. Decisions: the bottleneck replaces the verdict line inside the score card rather than adding a block; the free action fills the former locked slot and moves to the top of the right column; the share block absorbs "Share this result"; the landing card gets a compact tone toggle that swaps the verdict only; the two-line problem statement sits above the preview card.
