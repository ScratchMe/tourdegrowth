...
test("an unknown result never previews as a real image", async ({ request }) => {
  ...
  // not a broken route. Both LEGACY addresses (rewritten to the `legacy`
  // token, `next.config.mjs`) answer, and neither is cached as immutable:
  // the picture behind them can still change.
  for (const legacy of ["/r/sample/opengraph-image", "/r/sample/opengraph-image-1u74ed"]) {
    const sample = await request.get(legacy);
    expect(sample.status(), legacy).toBe(200);
    expect(sample.headers()["content-type"], legacy).toContain("image/png");
    expect(sample.headers()["cache-control"], legacy).not.toContain("immutable");
  }
});

/**
 * The share image's addresses, and why each must answer (REVIEW.md R-24,
 * then the version token of 2026-09-14).
 *
 ...
 * path hard-coded here that could drift from it. That URL carries a version
 * token (`lib/og/share-image.ts`) and is served as immutable — the whole
 * point of the change is that a repeat fetch never reaches Satori again, so
 * the cache header is asserted, not assumed. The two legacy addresses are
 * covered above.
 */
test("the OG image URL the page declares is the one that serves it, and it is cacheable", async ({ page, request }) => {
  await page.goto("/r/sample");
  const declared = await page.locator('meta[property="og:image"]').getAttribute("content");
  expect(declared).toBeTruthy();
  const declaredPath = new URL(declared!).pathname;
  expect(declaredPath).toMatch(/^\/r\/sample\/share\/[a-f0-9]{12}\.png$/);

  const image = await request.get(declaredPath + new URL(declared!).search);
  expect(image.status()).toBe(200);
  expect(image.headers()["content-type"]).toContain("image/png");
  expect(image.headers()["cache-control"]).toContain("immutable");
  expect(image.headers()["cache-control"]).toMatch(/s-maxage=\d{5,}/);

  // The picture in the share block is the same address — one render, one
  // cache entry — and so is the Save link.
  const card = page.getByTestId("share-card");
  expect(await card.locator("img").getAttribute("src")).toBe(declaredPath);
  expect(await card.getByRole("link", { name: /save/i }).getAttribute("href")).toBe(declaredPath);
});
 ...
 * never cached as immutable; a segment that is not a token at all is a 404.
 */
test("a stale token still previews, briefly cached; a non-token is a 404", async ({ request }) => {
  const stale = await request.get("/r/sample/share/000000000000.png");
  expect(stale.status()).toBe(200);
  expect(stale.headers()["content-type"]).toContain("image/png");
  expect(stale.headers()["cache-control"]).not.toContain("immutable");
  expect(stale.headers()["cache-control"]).toMatch(/s-maxage=\d+/);

  for (const bad of ["/r/sample/share/nope.png", "/r/sample/share/000000000000", "/r/sample/share/000000000000.jpg"]) {
